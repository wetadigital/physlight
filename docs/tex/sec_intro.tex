...
from the \gls{si} system, descibed in~\cite{bipm:si.2019}.
...
The reason is two-fold: on the one side radiant power includes \emph{all} radiation
...
If such a wide-band device existed it would conceivably capture energy of wavelengths
in the millimeters (say radar waves or far infrared waves) all the way to maybe
single nanometers\footnote{And for short wavelengths all sorts of
	problems appear, for example many materials such as flesh cease being opaque, or 
	scattering photons that much, a key trait for medical imaging} 
(which would be X-rays). Even without going to these extremes, even bands
quite near to the visible range look substantially different than they do in the
visible, as you can see in~\cref{fig:uv_ir_photo}. On the other side,
even in the visible range, our sensitivity to brightness exhibits a marked dependency
on the wavelength (as you can see in a plot of the luminous efficiency 
...
for the workings of human perception, called \textsl{photometric}. 
...
\begin{displaymath}
\Phi_v = K_{cd} \int \Phi_{e,\lambda}(\lambda)\; V(\lambda) \d\lambda.
\end{displaymath}
...
As such it defines\footnote{
	...
	literature~\parencite{Meyer-Arendt:68}.
...
$683\;[\unit{\lumen\per\watt}]$~\parencite{cie:018.2019,wyszecki2000}.
\fi
